...
from dataset import BWDataset, BWPairDataset, FusedDataset, build_training_transform, build_training_transform_with_reference, build_validation_transform
...
class Trainer(Base):
    def __init__(self, train_dataset, train_loader, val_dataset, val_loader, model, loss_fn, learning_rate, decay=DECAY, train_res=(256, 256), val_res=(256, 256), batch_size=32, val_batch_size=32, model_name='carn', writer=None):
        # self.model = model
...
def main():
    ...
    noref_train_dataset = BWDataset('data5/train', build_training_transform(train_res))
    ref_train_dataset = BWPairDataset('og', 'remastered', build_training_transform_with_reference(train_res), filterfunc=lambda name: int(name) % 10 != 0)
    train_dataset = FusedDataset([noref_train_dataset, ref_train_dataset])
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.val_batch_size, shuffle=True, num_workers=num_workers)

    noref_val_dataset = BWDataset('data5/val', build_validation_transform(val_res))
    ref_val_dataset = BWPairDataset('og', 'remastered', build_validation_transform(val_res), filterfunc=lambda name: int(name) % 10 == 0)
    val_dataset = FusedDataset([noref_val_dataset, ref_val_dataset])

    # TODO: parametrizable source resolution
    samples_per_val = np.ceil(1920/val_res[0])*np.ceil(1080/val_res[1])
    actual_val_batch_size = max(int(args.val_batch_size/samples_per_val), 1)
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=actual_val_batch_size, shuffle=True, num_workers=num_workers)

    trainer = Trainer(train_dataset, train_loader, val_dataset, val_loader, model, torch.nn.L1Loss(), args.learning_rate, batch_size=args.batch_size, val_batch_size=args.val_batch_size, writer=writer, model_name=args.name, decay=args.decay, train_res=train_res, val_res=val_res)
